...
if (max(time_scale_to_plot_minor)<end)
...
if (alt_back && colored!="backdrop")
	for (b in first_period:last_period)
		if (gtools::even(b))	rect(max(onset,time_scale_to_plot_minor[b]),mny,min(end,time_scale_to_plot_minor[b+1]),mxy,col=alt_back_hue,border=alt_back_hue)
...
if (use_strat_labels)	{
	...
	if (font=="")	{
		text(mid_minor[first_period:last_period],dummy_minor[first_period:last_period],label=strat_names_minor[first_period:last_period],cex=font_size);
		} else	{
		text(mid_minor[first_period:last_period],dummy_minor[first_period:last_period],label=strat_names_minor[first_period:last_period],cex=font_size,family=font);
		}
	...
	if (font=="")	{
		text(mid_major,dummy_major,label=strat_names_major,cex=font_size);
		} else	{
		text(mid_major,dummy_major,label=strat_names_major,cex=font_size,family=font);
		}
	}
}
...
if (mnn<mxn)	{
	...
	for (i in 1:length(numbers))
		axis(axe,at=log10(numbers[i]),tcl=-.3,labels=FALSE,lwd=0,lwd.ticks=linewd);
	}
for (i in 1:length(numbers))	{
	l <- numbers[i]
  if (font=="")	{

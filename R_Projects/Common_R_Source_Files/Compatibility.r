...
matrix_compatibility <- function(chmatrix,chtypes,chstates=NULL,UNKNOWN=-11,INAP=-22)	{
#compatibility_matrix(chmatrix,types,chtypes,UNKNOWN,INAP,fuzzy)
if (is.null(chstates)) chstates <- count_states(chmatrix);
chmatrix <- chmatrix[,chstates>0];	# added 2023-11-14
chstates <- chstates[chstates>0];  	# added 2023-11-14
return(sum(extract_off_diagonal(compatibility_matrix(chmatrix=char_matrix,types=chtypes,chstates=chstates,UNKNOWN=UNKNOWN,INAP=INAP))));
}

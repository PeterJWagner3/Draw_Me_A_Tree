...
principal_coordinates_analysis <- function(dist_matrix)	{
gowered <- gower_transform(dist_matrix);
pco <- eigen(gowered);
rownames(pco$vectors) <- rownames(dist_matrix);
nn <- nrow(dist_matrix)
colnames(pco$vectors) <-  paste("PO_",1:nn,sep="");
if (nn>99)	{
	colnames(pco$vectors)[1:9] <-  paste("PO_00",1:9,sep="");
	colnames(pco$vectors)[10:99] <-  paste("PO_0",10:99,sep="");
	} else if (nn>9)	{
	colnames(pco$vectors)[1:9] <-  paste("PO_0",1:9,sep="");
	}
names(pco) <- c("skree","vectors");
return(pco);
}
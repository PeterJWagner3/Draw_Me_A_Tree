...
tad1 <- sort(tad1[tad1>0],decreasing = TRUE);
tad2 <- sort(tad2[tad2>0],decreasing = TRUE);
spid1 <- names(tad1);
spid2 <- names(tad2);
S1 <- length(tad1);
S2 <- length(tad2);
...
f11 <- sum(names(tad1[tad1==1]) %in% names(tad2[tad2==1]));			# taxa found once each in both lists 1 & 2	#
f22 <- sum(names(tad1[tad1==2]) %in% names(tad2[tad2==2]));			# taxa found once each in both lists 1 & 2	#
D12 <- length(shared_taxa); # observed shared
X <- tad1[names(tad1) %in% shared_taxa];
Y <- tad2[names(tad2) %in% shared_taxa];
#for (a in 1:S1)	{
#	for (b in 1:S2)	{
#		if (spid1[a]==spid2[b])	{
#			shared[D12] <- spid1[a];
...
####Balsega's R code####
#This is taken from the paper's supplementray information and modified 
maxbibj<-function(pres_abs_data) {
## nsites is the number of rows
nsites<-nrow(pres_abs_data);
...
for(i in 1:(nsites-1)) {
	...
	## only in the ith and the jth site (row).
		bi <- sum(pres_abs_data[i,]&(!pres_abs_data[j,]));
		bj <- sum(pres_abs_data[j,]&(!pres_abs_data[i,]));
      ## We sum up 'max(bi,bj)' on every different pair of rows.
		sum.maxbibj <- sum.maxbibj+max(bi,bj);
		} # end comparison of site i with site j;
	} # end comparison of site i with other sites;
  
  ## We return the sum
return(sum.maxbibj);
}

minbibj<-function(pres_abs_data) {
...
zeroColumn<-function(pres_abs_data) {
taxon_presences <- colSums(pres_abs_data);
missing <- sum(taxon_presences==0);
if(missing!=0)
	warning(missing," missing species. This fact does not affect the result since total number of species is calculated excluding these columns.",call.=FALSE,immediate.=TRUE);
return(missing);
}

multisite_sorenson_beta <- function(pres_abs_data) {
## pres_abs_data must be a data frame
## Si is the number of species present in the ith site. We sum all  these values.
sumSi <- sum(pres_abs_data);	# all presences
...
## out.
St<-ncol(pres_abs_data)-zeroColumn(pres_abs_data);
  
## 'a' is the number of species shared for at least two sites
a <- sumSi-St;
index <- (minbibj(pres_abs_data)+maxbibj(pres_abs_data))/(minbibj(pres_abs_data)+maxbibj(pres_abs_data)+(2*a));

if(any(pres_abs_data>1))
	warning("The table contains values >1: the result may be meaningless.",call.=FALSE,immediate. = TRUE);
if(any(pres_abs_data<0))
	warning("The table contains values <0: the result may be meaningless.",call.=FALSE,immediate. = TRUE);  
return(index);
}

multisite_simpson_beta<-function(pres_abs_data) {
## pres_abs_data must be a data frame

## Si is the number of species present in the ith site. We sum all these values.
sumSi<-sum(pres_abs_data);
...
## 'a' is the number of species shared for at least two sites
a <- sumSi-St;
index<-minbibj(pres_abs_data)/(minbibj(pres_abs_data)+a);
...
multisite_nested_beta <-function(pres_abs_data) {
## pres_abs_data must be a data frame
...
index <- (a/(minbibj(pres_abs_data)+a))*((maxbibj(pres_abs_data)-minbibj(pres_abs_data))/((2*a)+maxbibj(pres_abs_data)+minbibj(pres_abs_data)));
if(any(pres_abs_data>1))
	warning("The table contains values >1: the result may be meaningless.",call.=FALSE,immediate. = TRUE);
if(any(pres_abs_data<0))
	warning("The table contains values <0: the result may be meaningless.",call.=FALSE,immediate. = TRUE);  

...
#tbins <- vector(length=nbins-1);
#for (tb in 1:(nbins-1))	tbins[tb] <- timescale[]	
d_bin <- b_bin-1;
ndy <- TRUE;
survived <- F;
while (ndy & d_bin<=nbins)	{
	d_bin <- d_bin+1;
	if (b_bin>=nbins)	{
...
#	print(paste(mus[d_bin],bint,ndy))
	if (is.nan(dpois(0,mus[d_bin]*bint)))	{
		print("WTF");
		print(paste(bday,d_bin,bint)); #mus[d_bin],
		print(timescale); #mus[d_bin],
#		print(paste(mus,d_bin,bint));
		bint <- 2;
		}
	if (is.nan(dpois(0,mus[d_bin]*bint)))
...
evolve_to_sampled_higher_taxon_richness_S_shifting_rates <- function(S,ps,qs,psis,ngenp,timescale,anagenesis=0,bifurcation=FALSE,temp_prec=0.01,debug=FALSE)	{
# cumulative and standing diversity
if (ngenp>0)	ngenp <- 1/ngenp;
...
timescale <- timescale-timescale[1]; #print("line 110");
bin_ends <- timescale[1:nbins]+tbins;
the_end <- bin_ends[nbins]; #timescale <- c(timescale,the_end);
birth <- rep(timescale[1],1);
death <- rep(simulate_extinction_date_given_variable_extinction_rates(bday=timescale[1],mus,timescale,tbins,temp_prec=temp_prec),1);
while (death[1]<=0)	death <- rep(simulate_extinction_date_given_variable_extinction_rates(bday=timescale[1],mus,timescale,tbins,temp_prec=temp_prec),1);
#print(death-birth)
duration <- death-birth;
birth_bin <- maxfinds <- dbr <- spc <- cumulative <- standing <- 1;
death_bin <- sum(death[1]>timescale);
g_sampled <- s_sampled <- c();	# list of sampled species
fa <- la <- occurrences <- 0;	# occurrences per species
all_finds <- matrix(0,1,maxfinds);
keep_going <- TRUE;
ancestral <- ttl_daughters <- dist_fr_founder <- sfound <- gfinds <- gfound <- 0;
taxon <- remaining <- tries <- cumulative <- ttl_sim <- ttl_sim_max <- genus_founder <- ngenera <- genus <- 1; #print("line 130");
new_genus <- staxa <- c();
while (keep_going)	{
	spc <- remaining[1];
	staxa <- unique(c(staxa,spc));
	if (spc>1)	{
		sd <- dist_fr_founder[ancestral[spc]]+1;
		ngen <- runif(1);
		if (ngen<=ngenp)	{
#		if (runif(1)<(1-((1-gamma)^sd)))	{
			ngenera <- 1+ngenera;
			if (spc>length(genus))	genus <- c(genus,rep(0,spc-length(genus)));
			genus[is.na(genus)] <- 0;
			genus[spc] <- ngenera;
			gfinds <- c(gfinds,0);
			genus_founder <- c(genus_founder,spc);
			if (spc>length(dist_fr_founder))	dist_fr_founder <- c(dist_fr_founder,rep(0,spc-length(dist_fr_founder)));
			dist_fr_founder[is.na(dist_fr_founder)] <- 0;
			} else	{
			if (spc>length(dist_fr_founder))	dist_fr_founder <- c(dist_fr_founder,rep(0,spc-length(dist_fr_founder)));
			dist_fr_founder[is.na(dist_fr_founder)] <- 0;
			dist_fr_founder <- c(dist_fr_founder,dist_fr_founder[spc]+1);
			if (spc>length(genus))	genus <- c(genus,rep(0,spc-length(genus)));
			genus[is.na(genus)] <- 0;
			genus[spc] <- genus[ancestral[spc]];
			}
		new_genus <- rbind(new_genus,c(spc,genus[spc],ancestral[spc],ngen))
		}
	duration <- abs((death[spc]-birth[spc]));
	mini_mes <- 0;
	if (anagenesis>0)	{
...
#		rownames(all_finds) <- 1:cumulative;
	...
	if (nfinds>0)	{
		sfound <- sfound+1;
		gfinds[is.na(gfinds)] <- 0;
		gfinds[genus[spc]] <- gfinds[genus[spc]]+1;
		if (gfinds[genus[spc]]==1)	gfound <- gfound+1;
		g_sampled <- (1:ngenera)[gfinds>0];
		if (maxfinds<nfinds)	{
	...
	if (length(bdates)>4)	{
		bdates <- bdates[order(runif(length(bdates)))[1:4]];
		bdates <- sort(bdates);
		}
	daughters <- length(bdates);
	#	bdates <- simulate_events_given_varying_event_rates_over_time(birth[spc])
	
	ttl_daughters[spc] <- ttl_daughters[spc]+daughters;
...
#			splits <- temp_prec*ceiling(splits/temp_prec)
			...
			for (i in 1:daughters)	{
				...
				ancestral <- c(ancestral,prior_anc,prior_anc);
				birth <- c(birth,splits[i],splits[i]);
				ttl_daughters <- c(ttl_daughters,0);
				if (i < daughters)  {
			...
			# update dimensions of information arrays
			remaining <- c(remaining,cumulative+(1:daughters))
			cumulative <- cumulative+daughters;
			added <- rep(0,daughters);
			ttl_daughters <- c(ttl_daughters,added);
			occurrences <- c(occurrences,added);
			fa <- c(fa,added);
			la <- c(la,added);
			all_finds <- rbind(all_finds,matrix(0,daughters,maxfinds));
...
#		standing <- tally_richness_from_continuous_ranges(ranges,temp_prec)
	...
	if (sum(all_finds[spc,]!=0)>0)	{
		...
		s_sampled <- unique(c(s_sampled,spc));
		}

	sfound <- length(s_sampled);
	if (debug)	print(sfound);
#	staxa <- 1:length(fa);
	if (length(remaining) > 1)	{
		remaining <- remaining[2:length(remaining)];
		remaining <- remaining[order(birth[remaining])];
		sampled_clade <- data.frame(staxa=staxa,genus=genus[staxa],ancestral=ancestral[staxa],birth=birth[staxa],death=death[staxa],fa=fa[staxa],la=la[staxa]);
	#	rownames(clade_so_far) <- c("staxa","genus","ancestral","birth","death","fa","la");
		if (taxon>1 & length(s_sampled)>0)	{
			sampled_clade <- sampled_clade[sampled_clade$fa>0,];
			sampled_clade <- sampled_clade[order(sampled_clade$fa),];
			sampled_clade <- sampled_clade[match(unique(sampled_clade$genus),sampled_clade$genus),];
			if (debug)	print(base::t(sampled_clade));
			}
		taxon <- taxon+1;
#		print(clade_so_far);
		}	else if (gfound < S)	{
#		print(paste(ngenera,gfound));
		tries <- tries+1;
		ttl_sim <- c(ttl_sim,spc);
		ttl_sim_max <- max(ttl_sim);
		dbr <- dbr+1;
		# reboot..... :-(     
		# reset variables to 9;
		dist_fr_founder <- sfound <- gfinds <- gfound <- ancestral <- ttl_daughters <- 0;
		# reset variables to 1;
		taxon <- birth_bin <- maxfinds <- remaining <- genus_founder <- ngenera <- genus <- spc <- cumulative <- standing <- 1;
		birth <- timescale[1];
		death <- rep(simulate_extinction_date_given_variable_extinction_rates(bday=timescale[1],mus,timescale,tbins,temp_prec = temp_prec),1);
		while (death[spc]<=1)	death <- rep(simulate_extinction_date_given_variable_extinction_rates(bday=timescale[1],mus,timescale,tbins,temp_prec = temp_prec),1);
		death_bin <- sum(death[spc]>timescale);
		duration <- c(death[spc]-birth[spc])
		# clear sampling vectors
		taxa <- s_sampled <- c();	# list of sampled species
		fa <- la <- occurrences <- vector(length=1)	# occurrences per species
		all_finds <- matrix(0,1,maxfinds)
		}
#	cbind(order(birth[remaining]),birth[remaining])
	...
	# stop when it is not possible to find any species with older fossils after we have S
	if (gfound>=S)	if (relv_fas[S]<min(birth[remaining]))	keep_going <- FALSE;
	}
species_richness <- hist(genus[genus>0],breaks=0:ngenera,plot=F)$counts;
#sampled_clade
#mean(species_richness);
#hist(species_richness,breaks=0:max(species_richness));
#plot(birth[fa>0][1:38],fa[fa>0])
# the simulations usually over-sample to avoid arbitrary cutoff wonkiness
#	So, reduce to just S species
fa_gen <- vector(length=length(g_sampled));
genotypes <- sort(sampled_clade$staxa);
if (nrow(sampled_clade)>S)		sampled_clade <- sampled_clade[1:S,];
#if (nrow(sampled_clade_gen)>S)	{
#	xx <- g_sampled[order(fa[g_sampled],decreasing=FALSE)]
#	s_sampled <- sort(xx[1:S],decreasing=FALSE)
#	}

venn_tree_raw <- accersi_raw_venn_tree_from_ancestor_list(sampled=sort(1:max(sampled_clade$staxa)),ancestral);
branchings <- rep(1,max(sampled_clade$staxa));
branchings[unique(ancestral[ancestral>0 & ancestral<max(sampled_clade$staxa)])] <- 0;
venn_tree_info <- condense_simulated_venn_tree(venn_tree_raw = venn_tree_raw,sampled_spc=sampled_clade$staxa,species_divergences=birth)
#venn_tree_info$Branchings;
#venn_tree_raw <- accersi_raw_venn_tree_from_ancestor_list(sampled_clade$staxa,ancestral);
#venn_tree_info <- accersi_simulated_venn_tree_info(sampled=sort(sampled_clade$staxa),ancestral);
venn_tree_raw <- venn_tree_info$Venn_Tree_Sampled;
venn_tree <- venn_tree_info$Venn_Tree;
all_branchings <- venn_tree_info$Branchings;
all_divergences <- venn_tree_info$Divergence_Times;
vector_tree <- transform_venn_tree_to_vector_tree(venn_tree);
matrix_tree <- transform_venn_tree_to_matrix_tree(venn_tree);
sampled_spc <- sort(sampled_clade$staxa);
ghost_ends <- c(fa[sort(sampled_spc)],rep(MAXNO,nrow(venn_tree)));
for (cl in nrow(venn_tree):1)	{
	htu <- S+cl;
	spcc <- matrix_tree[cl,matrix_tree[cl,]>0];
	ghost_ends[htu] <- max(all_divergences[spcc]);
	}
#cbind(all_divergences,ghost_ends);
# get ancestors; make sure that ancestor of node that gets collapsed is marked as ancestor of daughter but not node
output <- list(venn_tree,venn_tree_raw,all_divergences,ghost_ends,all_branchings,sampled_clade);
names(output) <- c("venn_tree","venn_tree_raw","divergences","lineage_ends","branchings","simulation_summary");
return(output);
}
...
evolve_to_sampled_higher_taxon_richness_S_shifting_rates_old <- function(S,ps,qs,psis,gamma,timescale,anagenesis=0,bifurcation=FALSE,temp_prec=0.01,debug=FALSE)	{
# cumulative and standing diversity
lambdas <- ps/(1+anagenesis);	# cladogenesis rate
mus <- qs/(1+anagenesis);			# extinction rate
darwin <- anagenesis*mean(mus);		# anagenesis rate;
...
ancestral <- c(0);
remaining <- c(1);
tries <- 1;	# this will keep track of how many species we have sampled
g_sampled <- s_sampled <- c();	# list of sampled species
fa <- la <- occurrences <- vector(length=1);	# occurrences per species
all_finds <- matrix(0,1,maxfinds);
keep_going <- TRUE;
ttl_sim <- ttl_sim_max <- 1.0;
dist_fr_founder <- sfound <- gfinds <- gfound <- 0;
ttl_daughters <- c(0);
genus_founder <- ngenera <- genus <- 1;
while (keep_going)	{
	spc <- remaining[1];
	duration <- abs((death[spc]-birth[spc]));
	mini_mes <- 0;
...
#		rownames(all_finds) <- 1:cumulative;
	...
	if (nfinds>0)	{
		sfound <- sfound+1;
		if (gfinds[genus[spc]]==0)	gfound <- gfound+1;
		gfinds[genus[spc]] <- gfinds[genus[spc]]+1;
		g_sampled <- (1:ngenera)[gfinds>0];
		if (maxfinds<nfinds)	{
			all_finds <- cbind(all_finds,array(0,dim=c(cumulative,nfinds-maxfinds)));
	...
	# second, simulate cladogenesis;
	bina <- sum(birth[spc]>timescale);
	binz <- sum(death[spc]>timescale);
	if (bina<(binz-1))	{
		span_b <- timescale[bina+1]-birth[spc];
		ex_b <- lambdas[bina]*(timescale[bina+1]-birth[spc]);
		ex_f <- psis[bina]*(timescale[bina+1]-birth[spc]);
		for (bb in (bina+1):(binz-1))	{
			ex_b <- c(ex_b,lambdas[bb]*tbins[bb]);
			ex_f <- c(ex_f,psis[bb]*tbins[bb]);
			span_b <- c(span_b,tbins[bb]);
			}
		span_b <- c(span_b,death[spc]-timescale[binz]);
		names(span_b) <- names(tbins[bina:binz]);
		ex_b <- c(ex_b,lambdas[binz]*(death[spc]-timescale[binz]));
		ex_f <- c(ex_f,psis[binz]*(death[spc]-timescale[binz]));
#		ex_r <- lambdas[(bina+1):(binz-1)]*timescale[(bina+1):(binz-1)];
#		ex_z <- lambdas[binz]*(death[spc]-timescale[binz]);
		} else if (bina==(binz-1))	{
		span_b <- timescale[bina+1]-birth[spc];
		span_b <- c(span_b,death[spc]-timescale[binz]);
		names(span_b) <- names(tbins[bina:binz]);
		ex_b <- c(lambdas[bina]*(timescale[bina+1]-birth[spc]),lambdas[binz]*(death[spc]-timescale[binz]));
		ex_f <- c(psis[bina]*(timescale[bina+1]-birth[spc]),psis[binz]*(death[spc]-timescale[binz]));
		} else	{
		span_b <- death[spc]-birth[spc];
		names(span_b) <- names(tbins[bina]);
		ex_b <- span_b*lambdas[bina];
		ex_f <- span_b*psis[bina];
		}
	daughters <- min(4,median(rpois(1,sum(ex_b))));
	dd <- 0;
	bdates <- c();
	if (daughters>0)	{
		ex_b_cum <- cumsum(ex_b)/max(cumsum(ex_b));
		span_b_cum <- c(0,cumsum(span_b));
		xx <- sort(runif(daughters));
		bdates <- c();
		for (dd in 1:daughters)	{
#			spc2 <- ttl_spc
			# see if "genification" happens
#			if (runif(1)<(1-(1-gamma)^(dist_fr_founder[spc]+1)))	{
			if (runif(1)<gamma)	{
				gfinds <- c(gfinds,0);
				ngenera <- 1+max(genus);
				genus_founder <- c(genus_founder,length(genus)+1);
				dist_fr_founder <- c(dist_fr_founder,0);
				genus <- c(genus,ngenera);
				} else	{
				genus <- c(genus,genus[spc]);
				dist_fr_founder <- c(dist_fr_founder,dist_fr_founder[spc]+1);
				}
			bb <- sum(xx[dd]>ex_b_cum)+1;
			bdates <- c(bdates,birth[spc]+span_b_cum[bb]+runif(1)*(span_b_cum[bb+1]-span_b_cum[bb]));
			}
	}	
#	hist(daughters,breaks=0:12)
#	bdates <- unique(simulate_events_given_varying_event_rates_over_time(bday=birth[spc],dday=death[spc],rates_over_time=lambdas,tscale=timescale,tbins,temp_prec = temp_prec));
	bdates <- bdates[bdates<the_end];
	bdates <- bdates[!is.na(bdates)];
	daughters <- length(bdates);
#	ttl_daughters <- c(ttl_daughters,daughters);
	ttl_daughters[spc] <- ttl_daughters[spc]+daughters;
	if (sum(bdates<birth[spc])>0)	{
		print("Call The Doctor!!!");
		break;
...
#		standing <- tally_richness_from_continuous_ranges(ranges,temp_prec)
	...
	if (length(remaining) > 1)	{
		remaining <- remaining[2:length(remaining)];
		remaining <- remaining[order(birth[remaining])];
		clade_so_far <- rbind(staxa,genus,ancestral,birth,death,fa,la);
#		print(clade_so_far);
		}	else if (gfound < S)	{
		tries <- tries+1;
		ttl_sim <- c(ttl_sim,spc);
		...
		dbr <- dbr+1
		# reboot..... :-(     
		ngenera <- genus <- spc <- cumulative <- standing <- 1
		birth <- rep(timescale[1],1);
		birth_bin <- 1;
		death <- rep(simulate_extinction_date_given_variable_extinction_rates(bday=timescale[1],mus,timescale,tbins,temp_prec = temp_prec),1);
		while (death[spc]<=1)	death <- rep(simulate_extinction_date_given_variable_extinction_rates(bday=timescale[1],mus,timescale,tbins,temp_prec = temp_prec),1);
		...
		remaining <- c(1);
		# clear sampling vectors
		s_sampled <- c();	# list of sampled species
		fa <- la <- occurrences <- vector(length=1)	# occurrences per species
		maxfinds <- 1;
		all_finds <- matrix(0,1,maxfinds)
		dist_fr_founder <- sfound <- gfinds <- gfound <- 0;
...
species_richness <- hist(genus,breaks=0:ngenera,plot=F)$counts;
#hist(species_richness,breaks=0:max(species_richness));
#plot(birth[fa>0][1:38],fa[fa>0])
# the simulations usually over-sample to avoid arbitrary cutoff wonkiness
#	So, reduce to just S species
fa_gen <- vector(length=length(g_sampled));
sampled_clade <- as.data.frame(base::t(clade_so_far));
sampled_clade <- sampled_clade[sampled_clade$fa>0,];
sampled_clade <- sampled_clade[order(sampled_clade$fa),];
sampled_clade_gen <- sampled_clade[unique(match(unique(sampled_clade$genus),sampled_clade$genus)),];
genotypes <- sort(sampled_clade_gen$staxa);
if (nrow(sampled_clade_gen)>S)		sampled_clade_gen <- sampled_clade_gen[1:S,];
#if (nrow(sampled_clade_gen)>S)	{
#	xx <- g_sampled[order(fa[g_sampled],decreasing=FALSE)]
#	s_sampled <- sort(xx[1:S],decreasing=FALSE)
#	}

venn_tree_info <- accersi_simulated_venn_tree_info(1:max(sampled_clade_gen$staxa),ancestral);
venn_tree_raw <- venn_tree_info$Venn_Tree;
sampled_spc <- sampled_clade_gen$staxa; branchings <- venn_tree_info$Branchings; species_divergences <- birth[1:max(sampled_clade_gen$staxa)];
#output <- condense_simulated_venn_tree(venn_tree_raw,sampled_spc=sampled_clade_gen$staxa,branchings=venn_tree_info$Branchings,species_divergences=birth);
...
logistic_to_standing_richness_S_plus <- function(R=1.1,mu=1.0,K=25,bifurcation=F,temp_prec = 0.1,post_exponential=1,max_life=MAXNO)	{
...
# routine to eliminate nodes with only one sampled descendant
#sampled_spc <- sampled_clade_gen$staxa; branchings <- venn_tree_info$Branchings; species_divergences <- birth[1:max(sampled_clade_gen$staxa)];
condense_simulated_venn_tree <- function(venn_tree_raw,sampled_spc,birth,species_divergences)	{
# written 2025-01-15
# venn_tree_raw: venn_tree for all simulated taxa;
#	sampled_spc: simulated sampled taxa.
#	species_divergences: true origination times of species
ancestors <- venn_tree_raw[,1];
branchings <- rep(1,max(venn_tree_raw));
branchings[ancestors] <- 0;
clade_divergences <- species_divergences[venn_tree_raw[,1]];

# condense step 1: eliminate unsampled clades
venn_tree <- venn_tree_raw;
names(clade_divergences) <- rownames(venn_tree) <- venn_tree_raw[,1];
raw_nodes <- nrow(venn_tree_raw);
clade_richness <- vector(length=raw_nodes);
for (cl in 1:raw_nodes)	{
	venn_tree[cl,!venn_tree[cl,] %in% sampled_spc] <- MAXNO;
	venn_tree[cl,] <- sort(venn_tree[cl,]);
	venn_tree[cl,venn_tree[cl,] %in% MAXNO] <- 0;
#	clade_richness[cl] <- sum(venn_tree[cl,]!=0);
#	clade_sums[cl] <- sum(venn_tree[cl,]);
	}
clade_richness <- tally_clade_richness_from_venn_tree(venn_tree);
clade_sums <- rowSums(venn_tree);

venn_tree <- venn_tree[clade_richness>0,];
venn_tree <- venn_tree[,colSums(venn_tree)>0];
clade_divergences <- clade_divergences[clade_richness>0];
clade_richness <- clade_richness[clade_richness>0];
clade_sums <- rowSums(venn_tree);

# condense step 2: eliminate singleton clades & duplicate clades
# 	condense branchings & divergence times as you go.
init_nodes <- nrow(venn_tree);
clade_branchings <- rep(1,init_nodes);
names(clade_branchings) <- rownames(venn_tree);
#for (cl in init_nodes:2)	{
cl <- init_nodes;
while (cl>1)	{
	if (clade_richness[cl]==1)	{
		spc <- venn_tree[cl,1];
		branchings[spc] <- branchings[spc]+1;
		species_divergences[spc] <- clade_divergences[cl];
		clade_branchings[cl] <- 0;
		} else	{
		cll <- cl-1;
		duplicates <- max((1:cll)*(clade_richness[1:cll]==clade_richness[cl])*(clade_sums[1:cll]==clade_sums[cl]));
		if (duplicates>0)	{
			# add branchings preceding identically sampled daughter node
			clade_branchings[duplicates] <- clade_branchings[duplicates]+clade_branchings[cl];
			clade_branchings[cl] <- clade_richness[cl] <- clade_sums[cl] <- 0;
			# we'll erase this node, so don't bother updating clade_divergences;
			}
		}
	cl <- cl-1;
	}
clade_divergences <- clade_divergences[clade_branchings>0];
venn_tree <- venn_tree[clade_branchings>0,];
clade_branchings <- clade_branchings[clade_branchings>0];
otu_branchings <- branchings[sort(sampled_spc)];
names(otu_branchings) <- venn_tree[1,];
red_venn_tree <- venn_tree;
for (i in 1:nrow(venn_tree))	red_venn_tree[i,red_venn_tree[i,]>0] <- match(red_venn_tree[i,red_venn_tree[i,]>0],sampled_spc);
all_branchings <- c(otu_branchings,clade_branchings);
sampled_divergences <- species_divergences[sort(sampled_spc)];
names(sampled_divergences) <- 1:length(sampled_divergences);
names(clade_divergences) <- length(sampled_divergences)+(1:length(clade_divergences));
all_divergences <- c(sampled_divergences,clade_divergences);
names(all_branchings) <- names(all_divergences);
output <- list(venn_tree,red_venn_tree,all_branchings,all_divergences)
names(output) <- c("Venn_Tree_Sampled","Venn_Tree","Branchings","Divergence_Times");
return(output);
}

tally_clade_richness_from_venn_tree <- function(venn_tree,min_taxon=1)	{
clade_richness <- vector(length=nrow(venn_tree));
for (cl in 1:nrow(venn_tree))	clade_richness[cl] <- sum(venn_tree[cl,]>=min_taxon);
return(clade_richness);
}
...
needy_brs <- unique_ab[branch_changes[unique_ab]==0];
counter <- b <- 0;
dbranches <- ncol(pabm);	# number of alterable branches
...
#	ch <- ceiling(nchars*runif(1));
	...
	now_changed <- c(pabm[ch,1:char_changes[ch]],br);
	to_change <- pabm[ch,(char_changes[ch]+1):dbranches][!pabm[ch,(char_changes[ch]+1):dbranches] %in% br];
	pabm[ch,] <- as.numeric(c(now_changed,to_change));
...
# get a matrix with the same compatibility as an observed matrix
evolve_discrete_characters_to_observed_compatibility_punctuated <- function(init_chmatrix,venn_tree,branchings,chtypes,ttl_compat=NULL,nstates=NULL,UNKNOWN=-11,INAP=-22,repl=-1,full_output=FALSE)	{
# ttl_compat: compatibility to which to evolve
# venn_tree: a tree giving all of the observed descendants (direct & indirect)
# branchings: length of each branch
# nstates: number of states for each character
...
#		if (branch_changes[br]==0)	{
	...
	ch <- mbap[1,1];  # grab the "earliest" change that should have happened.
	a <- 1;
	while (char_changes[ch]>=mx_ch[ch])	{
		a <- a+1;
		ch <- mbap[a,1];  # grab the "earliest" change that should have happened.
		}
#	ch <- ceiling(nchars*runif(1));
	# shuffle branch forward to be the nth change for that character
	now_changed <- c(pabm[ch,1:char_changes[ch]],br);
	to_change <- pabm[ch,(char_changes[ch]+1):dbranches][!pabm[ch,(char_changes[ch]+1):dbranches] %in% br];
	if (length(c(now_changed,to_change))!=ncol(pabm))	{
		print(c(now_changed,to_change));
		print(pabm[ch,]);
		}
	pabm[ch,] <- as.numeric(c(now_changed,to_change));
#	pabm[ch,] <- as.numeric(c(c(pabm[ch,1:char_changes[ch]],br),pabm[ch,][!pabm[ch,] %in% c(pabm[ch,1:char_changes[ch]],br)]));
...
evolve_discrete_characters_to_observed_compatibility_continuous <- function(init_chmatrix,venn_tree,branch_durations,chtypes,ttl_compat=NULL,nstates=NULL,UNKNOWN=-11,INAP=-22,repl=-1,full_output=F,every_branch=FALSE)	{
# ttl_compat: compatibility to which to evolve
# venn_tree: a tree giving all of the observed descendants (direct & indirect)
# branchings: length of each branch
# nstates: number of states for each character
...
simchmatrix <- init_chmatrix;
for (ch in 1:nchars)	simchmatrix[simchmatrix[,ch]>=0,ch] <- 0;
branchings <- rep(0,length(branch_durations));
branchings[branch_durations>0] <- 1;
...
bcc <- matrix(0,nchars,fb);	# branch changes per character
mxst <- max(nstates)+1;
st_rich <- matrix(0,nchars,mxst)
mx_ch <- vector(length=nchars)
for (ch in 1:nchars)	{
	charstates <- init_chmatrix[,ch];
	polys <- charstates[charstates<0 & !charstates %in% c(UNKNOWN,INAP)];
	charstates <- charstates[!charstates %in% c(UNKNOWN,INAP,polys)];
	st_rich[ch,] <- hist(charstates,breaks=-1:mxst,plot=FALSE)$counts[1:mxst];
	pm <- 0;
	while (pm < length(polys))	{
		pm <- pm+1;
		st_rich[ch,unravel_polymorph(polys[pm])+1] <- st_rich[ch,unravel_polymorph(polys[pm])+1]+1/length(unravel_polymorph(polys[pm]));
		}
#	for (s in 1:notu)	{
#		if (chmatrix[s,ch]!=UNKNOWN && chmatrix[s,ch]!=INAP)	st_rich[ch,1] <- st_rich[ch,1]+1
#		if (!init_chmatrix[s,ch] %in% c(UNKNOWN,INAP))	st_rich[ch,1] <- st_rich[ch,1]+1;
#		}
	mx_ch[ch] <- round(sum(st_rich[ch,])-max(st_rich[ch,])+0.001,0);
	#if (nstates[ch]>=2)	mx_ch[ch] <- st_rich[ch,1]
	}
...
#for (n in 1:nodes)  rich[n] <- sum(venn_tree[n,]>0);
#for (n in 1:nodes)  rich[n] <- length(subset(venn_tree[n,],venn_tree[n,]>0))
pabm <- get_possible_branches_for_all_characters(simchmatrix=simchmatrix,branchings=branchings,available_branches=available_branches,venn_tree=venn_tree)
#	simchmatrix,branchings,,venn_tree); # possible available branches matrix
pcc <- vector(length=nchars);  # possible character changes

# set up branch change order for characters (pabm);
abranches <- length(available_branches);
for (ch in 1:nchars)  {
	x <- unique(permute(subset(pabm[ch,],pabm[ch,]>0)));
	pcc[ch] <- length(x);
	for (c in 1:pcc[ch])  		pabm[ch,c] <- x[c];
	if (pcc[ch]<abranches)   for (c in (pcc[ch]+1):abranches)   pabm[ch,c] <- 0;
	}

# make a character by branch matrix giving probability of character changing on that branch
pcharch_on_branch <- array(0,dim=c(nchars,abranches)); # prob. character change on each branch;
colnames(pcharch_on_branch) <- available_branches;
for (ch in 1:nchars)	{
	nn <- cumsum(branch_durations[sort(pabm[ch,pabm[ch,]>0])])/sum(branch_durations[sort(pabm[ch,pabm[ch,]>0])])
	pcharch_on_branch[ch,match(names(nn),colnames(pcharch_on_branch))] <- nn;
	for (br in 2:abranches)	if (pcharch_on_branch[ch,br]==0)	pcharch_on_branch[ch,br] <- pcharch_on_branch[ch,br-1];
	}
...
for (ch in 1:nchars)	{
	if (nstates[ch]>=2)	{
		prior_compat <- simcompatmat[ch,];
    use_br <- vector(length=nstates[ch]-1);
    for (ub in 1:length(use_br))	{
    	x <- runif(1);
    	use_br[ub] <- as.numeric(colnames(pcharch_on_branch)[1+sum(x>pcharch_on_branch[ch,])]);
    	}
		for (c in 1:(nstates[ch]-1))	{
			br <- use_br[c]
			if (br<=notu)	{
...
counter <- b <- 0;
dbranches <- ncol(pabm);	# number of alterable branches
while (b < length(needy_brs) & every_branch)	{
	b <- b+1;
	br <- needy_brs[b];	### make sure that is in all such routines!!!!
#		if (branch_changes[br]==0)	{
	counter <- counter+1;
...
#			br <- pabm[ch,c]
    	x <- runif(1);
    	br <- as.numeric(colnames(pcharch_on_branch)[1+sum(x>pcharch_on_branch[ch,])]);
			}
		prior_char <- simchmatrix[,ch]
		prior_compat <- simcompatmat[ch,]
...
#			if (simchmatrix[s,ch]!=UNKNOWN && simchmatrix[s,ch]!=INAP)	{
			if (!simchmatrix[s,ch] %in% c(UNKNOWN,INAP) & !is.polymorph(simchmatrix[s,ch]))	{
				pabm[ch,a]<-s;
				a <- a+1;

...
prob_N_descendants_given_origination_and_extinction <- function(N,lambda,mu)	{
#dpois(N,lambda*chrons) * (1-probability_of_lineage_extinction_given_extinction_time_and_rate(chrons,mu));
#chrons <- 100;
return(integrate(prob_N_descendants_given_time_origination_and_extinction,lower=0,upper=100,N=N,lambda=lambda,mu=mu)$value)
}

prob_N_descendants_given_time_origination_and_extinction <- function(chrons,N,lambda,mu)	{
#print(dpois(N,lambda*chrons))
return(dpois(N,lambda*chrons) * (1-probability_of_lineage_extinction_given_extinction_time_and_rate(chrons,mu)));
#return((dpois(N,lambda*chrons) * (1-probability_of_lineage_extinction_given_extinction_time_and_rate(chrons,mu)))/integrate(probability_of_lineage_extinction_given_extinction_time_and_rate,lower=0,upper=100,mu=mu)$value);
}
...
colnames(taxon_ranges)[colnames(taxon_ranges) %in% c("FA","FAD","interval_lb","first_interval")] <- "fa";
colnames(taxon_ranges)[colnames(taxon_ranges) %in% c("LA","LAD","interval_ub","last_interval")] <- "la";
taxon_ranges <- taxon_ranges[taxon_ranges$fa!=0,];
if (is.null(taxon_ranges$taxon))	{
	taxa <- rownames(taxon_ranges);
	} else	{
	taxa <- taxon_ranges$taxon;
	}
...
# written 2025-05-14
accersi_three_timer_stats_from_pres_abs_matrix <- function(pres_in_each_interval)	{
intervals <- colnames(pres_in_each_interval);
nbins <- length(intervals);
three_timers_gap <- vector(length=nbins);
names(three_timers_gap) <- intervals;
three_timers_obs_pl <- three_timers_gap_pl <- two_timers_fore_pl <- two_timers_back_pl <- two_timers_fore <- two_timers_back <- three_timers_obs <- three_timers_gap;
for (b in 2:(nbins-1))	{
	z <- b-2;
	a <- b-1;
	c <- b+1;
	d <- b+2;
#	pres_in_each_interval[(pres_in_each_interval[,nbb]>0 & pres_in_each_interval[,nb]>0),]
#	pres_in_each_interval[(pres_in_each_interval[,nb]>0 & pres_in_each_interval[,bnb]>0),]
	if (b<nbins & b>1)	{
		tt_strict_test <- pres_in_each_interval[rowSums(pres_in_each_interval[,a:c])>0,a:c]
		two_timers_fore[b] <- sum(rowSums(tt_strict_test[,2:3])==2);
		two_timers_back[b] <- sum(rowSums(tt_strict_test[,1:2])==2);
		three_timers <- tt_strict_test[(tt_strict_test[,1]+tt_strict_test[,3])==2,]
		three_timers_obs[b] <- sum(three_timers[,2]);
		three_timers_gap[b] <- nrow(three_timers)-three_timers_obs[b];
		if (b>2 & b<(nbins-1))	{
			tt_loose_test_init <- pres_in_each_interval[rowSums(pres_in_each_interval[,z:d])>0,z:d]
			if (nrow(tt_loose_test_init)>0)	{
				three_timers_loose <- cbind(rowMaxs(tt_loose_test_init[,1:2]),tt_loose_test_init[,3],rowMaxs(tt_loose_test_init[,4:5]));
				rownames(three_timers_loose) <- rownames(tt_loose_test_init);
				} else	{
				three_timers_loose <- array(0,dim=c(2,3));
				}
			} else if (b>2)	{
			tt_loose_test_init <- pres_in_each_interval[rowSums(pres_in_each_interval[,z:c])>0,z:c]
			three_timers_loose <- cbind(rowMaxs(tt_loose_test_init[,1:2]),tt_loose_test_init[,3],tt_loose_test_init[,4]);
			rownames(three_timers_loose) <- rownames(tt_loose_test_init);
			} else if (b<(nbins-1))	{
			tt_loose_test_init <- pres_in_each_interval[rowSums(pres_in_each_interval[,a:d])>0,a:d]
			if (nrow(tt_loose_test_init)>0)	{
				three_timers_loose <- cbind(tt_loose_test_init[,1],tt_loose_test_init[,2],rowMaxs(tt_loose_test_init[,3:4]));
				rownames(three_timers_loose) <- rownames(tt_loose_test_init);
				three_timers_loose <- three_timers_loose[three_timers_loose[,1]>0,];
				three_timers_loose <- three_timers_loose[three_timers_loose[,3]>0,];
				} else	three_timers_loose <- array(0,dim=c(2,3));
			}
		two_timers_fore_pl[b] <- sum(rowSums(three_timers_loose[,2:3])==2);
		two_timers_back_pl[b] <- sum(rowSums(three_timers_loose[,1:2])==2);
		three_timers_loose <- three_timers_loose[three_timers_loose[,1]>0,];
		three_timers_loose <- three_timers_loose[three_timers_loose[,3]>0,];
		if (length(three_timers_loose)==3)	three_timers_loose <- array(three_timers_loose,dim=c(1,3));
		three_timers_obs_pl[b] <- sum(three_timers_loose[,2]);
		three_timers_gap_pl[b] <- nrow(three_timers_loose)-sum(three_timers_loose[,2]);
		}
	}
single_binners <- pres_in_each_interval[rowSums(pres_in_each_interval)==1,];
one_timer <- colSums(single_binners);
alroy_stats <- data.frame(interval=as.character(colnames(pres_in_each_interval)),
						  one_timer=as.numeric(one_timer),
						  two_timer_fore=as.numeric(two_timers_fore),
						  two_timer_back=as.numeric(two_timers_back),
						  three_timer_obs=as.numeric(three_timers_obs),
						  three_timer_gap=as.numeric(three_timers_gap),
						  two_timer_fore_pl=as.numeric(two_timers_fore_pl),
						  two_timer_back_pl=as.numeric(two_timers_back_pl),
						  three_timer_obs_pl=as.numeric(three_timers_obs_pl),
						  three_timer_gap_pl=as.numeric(three_timers_gap_pl));
return(alroy_stats);
}
...
probability_of_lineage_extinction_given_extinction_time_and_rate <- function(chrons,mu)	{
return(1-exp(-mu*chrons))
}

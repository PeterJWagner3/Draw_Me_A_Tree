...
timescale <- gradstein_2020_emended$time_scale;
...
mctree_file2 <- paste(tree_drawing_folder,"max_clade_cred_tree_FBDMM_alt.tre",sep="");
...
mctree1_info <- read_newick_string_mcmc_numbered(newick_string_full,otu_names);

mctree2 <- scan(file=mctree_file2,what=character(),sep="\n");
mctree2_info <- read_newick_string_with_branch_lengths(newick_string=mctree2,taxon_names=otu_names_nex);
...
#		rock_database$province[match(otu_sites$rock_no_sr[otu_sites$rock_no_sr>0],rock_database$rock_no)]
		...
		fuzzy_ranges$region[sp] <- paste(unique(rock_database$region[match(otu_sites$rock_no_sr[otu_sites$rock_no_sr>0],rock_database$rock_no)]),collapse=",");
		fuzzy_ranges$realm[sp] <- paste(unique(rock_database$realm[match(otu_sites$rock_no_sr[otu_sites$rock_no_sr>0],rock_database$rock_no)]),collapse=",");
		}
	}
...
finest_chronostrat <- timescale[timescale$scale==stratigraphic_scale,];
...
#mctree1_info <- read_newick_string_mcmc_beast(newick_string_full,otu_names);
clade_posteriors <- mctree1_info$clade_posteriors;
mctree1_info$ancestral <- mctree1_info$ancestral[!is.na(names(mctree1_info$ancestral))];
sampled_ancestors <- mctree1_info$ancestral;
notu <- match(-1,mctree1_info$vector_tree)-1;
...
basal_date_raw <- max(((mctree1_info$hpd$lb+mctree1_info$hpd$ub)/2)[1:notu]);	# base from Bayesian tree before rescaling 0 to actual upper bound
basal_taxon_no <- match(basal_date_raw,((mctree1_info$hpd$lb+mctree1_info$hpd$ub)/2)[1:notu]);
...
#faux_recent <- min(fuzzy_ranges$fa_ub);
hpd_age <- -(mctree1_info$hpd_age+faux_recent); # upper & lower bound of FA;
...
vector_tree <- mctree1_info$vector_tree;
#cbind(vector_tree,vector_tree2)
#sum(vector_tree!=vector_tree2)
branch_durations <- branch_durations2;

#cbind(vector_tree,vector_tree2);
mat_tree <- transform_vector_tree_to_matrix_tree(vector_tree=mctree1_info$vector_tree);
...
branch_ranges <- branch_dates+(hpd_age$md[1]-branch_dates$finish[1]);
rownames(branch_ranges) <- rownames(hpd_age);
...
#nNodes <- nrow(mat_tree)
phylo_axis <- get_phylo_axis_from_newick_string_w_anagenesis(newick_string = mctree1_info$newick,sampled_ancestors=sampled_ancestors,anagenetic_ancestors=anagenetic_ancestors);
...
time_scale_to_plot[1] <- max(time_scale_to_plot[1],-ceiling(max(abs(branch_ranges$start))/min(yearbreaks))*min(yearbreaks));
time_scale_to_plot[length(time_scale_to_plot)] <- min(max(time_scale_to_plot),-floor(min(abs(branch_ranges$finish))/min(yearbreaks))*min(yearbreaks));
...
phylo_axis <- phylo_axis-min(phylo_axis);
y_adj <- -max(phylo_axis)/20;
mxy <- max(phylo_axis)+y_adj;
mny <- 0;
ysize <- 7.5;
phylo_axis <- phylo_axis+y_adj;
...
#
boundary <- timescale$ma_lb[match("Carboniferous",timescale$interval)];

...
organize_by_parent_taxon <- FALSE;
...
rank_to_plot <- "family";	# choose which taxonomic rank you want to print; irrelevant if you don't choose variable_to_plot <- "taxonomy"
...
#mctree_file <- paste(tree_drawing_folder,"Paracrinoids.tre",sep="");
ingroup <- "Diploporita";
strat_rank <- "Stage";		# I recommend either "Stage" or "Substage"
stratigraphic_scale <- "International";		# I recommend either "International" or "Stage Slice"
...
taxon_information_file <- "";
ingroup <- "Strophomenoidea";
rank_to_plot <- "family";
range_file <- paste(tree_drawing_folder,"Strophomenoidea_Fuzzy_Ranges.xlsx",sep="");
mctree_file <- paste(tree_drawing_folder,"Strophomeonoidea_MCT_continuous.tre",sep="");
split_outgroups <- FALSE;
ingroup_rank <- pbdb_taxonomy$accepted_rank[match(ingroup,pbdb_taxonomy$taxon_name)]
study_end <- "Ordovician";
study_ma_ub <- time_scale$ma_ub[time_scale$interval==study_end];
...
mctree_file <- paste(tree_drawing_folder,"post_rj_Acanthoceratidae_150K_MCCT.tre",sep="");
...
ingroup <- "Baculitidae";
mctree_file <- paste(tree_drawing_folder,"rj_Baculitidae_MCT.trees",sep="");
range_file <- paste(tree_drawing_folder,"Baculitidae_Range_Data.xlsx",sep="");
ingroup_rank <- pbdb_taxonomy$accepted_rank[match(ingroup,pbdb_taxonomy$taxon_name)];
rank_to_plot <- "genus";
strat_rank <- "Stage";		# I recommend either "Stage" or "Substage"
stratigraphic_scale <- "International";		# I recommend either "International" or "Stage Slice"
study_end <- "Maastrichtian";
study_ma_ub <- time_scale$ma_ub[time_scale$interval==study_end];
use_PBBD_for_cooccr_anc_desc <- FALSE;

ingroup <- "Desmoceratoidea";
mctree_file <- paste(tree_drawing_folder,"post_rj_Desmoceratidae_MCC.tre",sep="");
range_file <- paste(tree_drawing_folder,"Desmoceratidae_Range_Data.xlsx",sep="");
taxon_information_file <- "";
ingroup_rank <- pbdb_taxonomy$accepted_rank[match(ingroup,pbdb_taxonomy$taxon_name)]
rank_to_plot <- "family";
rank_to_plot <- "subfamily";
...
if (taxon_information_file=="")  {
	...
	if (ingroup_rank %in% colnames(pbdb_taxonomy))	{
		...
		ingroup_taxonomy <- rbind(ingroup_taxonomy,pbdb_taxonomy[pbdb_taxonomy$taxon_name %in% total_ingroup$daughter,]);
		ti <- 0;
		while (ti < nrow(total_ingroup))	{
			ti <- ti+1;
			ingroup_taxonomy <- rbind(ingroup_taxonomy,pbdb_taxonomy[pbdb_taxonomy[,total_ingroup$daughter_rank[ti]] %in% total_ingroup$daughter[ti],]);
			}
...
#			ingroup_taxonomy <- rbind(ingroup_taxonomy,pbdb_taxonomy[pbdb_taxonomy$taxon_name==fuzzy_ranges$taxon[tx],])
	...
	if (rank_to_plot=="subfamily")
		otu_information$subfamily <- ingroup_taxonomy$subfamily[match(otu_information$taxon,ingroup_taxonomy$taxon_name)];
  ...
  otu_information$taxon_type <- rep("ingroup",notu);
	...
	informals <- (1:notu)[is.na(otu_information$genus)];
	add_these <- match(otu_information$taxon[informals],pbdb_taxonomy$taxon_name)
	ingroup_taxonomy <- rbind(ingroup_taxonomy,pbdb_taxonomy[add_these[!is.na(add_these)],]);
	otu_information$genus <- ingroup_taxonomy$genus[match(otu_information$taxon,ingroup_taxonomy$taxon_name)];
	if (rank_to_plot=="subfamily")	otu_information$subfamily <- ingroup_taxonomy$subfamily[match(otu_information$taxon,ingroup_taxonomy$taxon_name)];
	otu_information$family <- ingroup_taxonomy$family[match(otu_information$taxon,ingroup_taxonomy$taxon_name)];
  otu_information$order <- ingroup_taxonomy$order[match(otu_information$taxon,ingroup_taxonomy$taxon_name)];
  otu_information$class <- ingroup_taxonomy$class[match(otu_information$taxon,ingroup_taxonomy$taxon_name)];
  otu_information$taxon_type <- rep("ingroup",notu);
	...
	informals <- (1:notu)[is.na(otu_information$genus)];
	if (!is.null(fuzzy_ranges$proper_name))	{
		fuzzy_ranges$proper_name <- gsub("_"," ",fuzzy_ranges$proper_name);
		otu_information$genus[informals] <- ingroup_taxonomy$genus[match(fuzzy_ranges$proper_name[informals],ingroup_taxonomy$taxon_name)];
		if (rank_to_plot=="subfamily")	otu_information$subfamily[informals] <- ingroup_taxonomy$subfamily[match(fuzzy_ranges$proper_name[informals],ingroup_taxonomy$taxon_name)];
		otu_information$family[informals] <- ingroup_taxonomy$family[match(fuzzy_ranges$proper_name[informals],ingroup_taxonomy$taxon_name)];
		otu_information$order[informals] <- ingroup_taxonomy$order[match(fuzzy_ranges$proper_name[informals],ingroup_taxonomy$taxon_name)];
		otu_information$class[informals] <- ingroup_taxonomy$class[match(fuzzy_ranges$proper_name[informals],ingroup_taxonomy$taxon_name)];
		}
	bg <- 0;
	informals <- (1:notu)[is.na(otu_information$genus)];
	while (bg < length(informals))	{
		bg <- bg+1;
		tx <- informals[bg];
		otu_information$genus[tx] <- divido_genus_names_from_species_names(species_name=otu_information$taxon[tx]);
		otu_information$family[tx] <- pbdb_taxonomy$family[match(otu_information$genus[tx],pbdb_taxonomy$taxon_name)];
		if (is.na(otu_information$family[tx]))	{
			otu_information$subfamily[tx] <- ingroup_taxonomy$subfamily[match(otu_information$genus[tx],ingroup_taxonomy$taxon_name)];
			otu_information$family[tx] <- ingroup_taxonomy$family[match(otu_information$genus[tx],ingroup_taxonomy$taxon_name)];
			otu_information$order[tx] <- ingroup_taxonomy$order[match(otu_information$genus[tx],ingroup_taxonomy$taxon_name)];
			otu_information$class[tx] <- ingroup_taxonomy$class[match(otu_information$genus[tx],ingroup_taxonomy$taxon_name)];
			} else	{
			otu_information$order[tx] <- pbdb_taxonomy$order[match(otu_information$genus[tx],pbdb_taxonomy$taxon_name)];
			otu_information$class[tx] <- pbdb_taxonomy$class[match(otu_information$genus[tx],pbdb_taxonomy$taxon_name)];
			otu_information$taxon_type[tx] <- "ingroup";
			}
		}
...
otu_information$family[otu_information$subfamily=="Puzosiinae"] <- "Puzosiidae";
rank_to_plot <- "family";
...
if (!beast) {
	tree_output <- read_newick_string_mcmc(newick_string_full,otu_names=otu_names_nex);
	} else	{
	tree_output <- read_newick_string_mcmc_numbered(newick_string_full,otu_names);
	}
...
#mctree2_info <- read_newick_string_with_branch_lengths(newick_string=mctree);
...
#htu_heights <- tree_output$htu_heights;
#nhtu <- length(vector_tree2);

#br_lngth_history <- array(0,dim=c(notu,1+max(htu_heights)));
#rownames(br_lngth_history) <- names(vector_tree)[1:notu];
#for (sp in 1:notu)	{
...
problem_taxa <- (1:notu)[round(branch_ranges$start[1:notu],5)>round(branch_ranges$finish[1:notu],5)];
...
#	cinctan_finds <- read.csv(paste(this_directory,"Cincta_Finds.csv",sep=""),header=TRUE,stringsAsFactors = FALSE);
	for (an in 1:length(ancestral_spc))	{
      ...
      anagenetic_ancestors[anc] <- 1;
		...
		if (use_PBBD_for_cooccr_anc_desc && anagenetic_ancestors[anc]==1)	{
			anc_find_data <- pbdb_finds[pbdb_finds$accepted_name==otu_names[anc],];
			if (nrow(anc_find_data)==0)	{
				if (!is.na(match(otu_names[anc],ingroup_taxonomy$taxon_name)))
					anc_find_data <- pbdb_finds[pbdb_finds$accepted_name %in% ingroup_taxonomy$accepted_name[match(otu_names[anc],ingroup_taxonomy$taxon_name)],]
				if (nrow(anc_find_data)==0)
					anc_find_data <- pbdb_finds[pbdb_finds$identified_name==otu_names[anc],];
				if (nrow(anc_find_data)==0)	{
					fix1 <- gsub(" n sp "," n. sp. ",otu_names[anc]);
					fix2 <- gsub(" sp "," sp. ",otu_names[anc]);
					anc_find_data <- pbdb_finds[pbdb_finds$identified_name %in% c(fix1,fix2),];
					}
				}
			
			for (pr in 1:length(progeny))	{
				this_f1_data <- pbdb_finds[pbdb_finds$accepted_name %in% otu_names[progeny[pr]],]
				if (nrow(this_f1_data)==0 && !is.na(match(otu_names[progeny[pr]],ingroup_taxonomy$taxon_name)))
					this_f1_data <- pbdb_finds[pbdb_finds$accepted_name %in% ingroup_taxonomy$accepted_name[match(otu_names[progeny[pr]],ingroup_taxonomy$taxon_name)],];
				if (nrow(this_f1_data)==0)	{
					fixa <- gsub(" n sp "," n. sp. ",otu_names[progeny[pr]]);
					fixb <- gsub(" sp "," sp. ",otu_names[progeny[pr]]);
					this_f1_data <- pbdb_finds[pbdb_finds$identified_name %in% c(fixa,fixb),];
					}
				if (pr==1)	{
					f1_data <- this_f1_data;
					} else	{
					f1_data <- rbind(f1_data,this_f1_data);
					}
				}
			if (nrow(f1_data)>0 && nrow(anc_find_data)>0)	{
				coccur_mat <- accersi_cooccurence_matrix(rbind(anc_find_data,f1_data));
				if (sum(coccur_mat[1,2:ncol(coccur_mat)])>0)
					anagenetic_ancestors[anc] <- 0;
				}
...
#				missing_progeny <- otu_names[progeny][!otu_names[progeny] %in% pbdb_finds$accepted_name]
#				mp <- 0;
#				while (mp < length(missing_progeny) & !is.null(fuzzy_ranges$proper_name))	{
#					mp <- mp+1;
#					txn <- match(missing_progeny,fuzzy_ranges$taxon);
#					taxon_name <- fuzzy_ranges$proper_name[txn];
#					poss_finds <- pbdb_finds[pbdb_finds$accepted_name %in% taxon_name,];
#					poss_sites <- pbdb_sites[pbdb_sites$collection_no %in% poss_finds$collection_no,];
#					poss_sites$region <- rock_unit_data$rock_unit_database$region[match(poss_sites$rock_no_sr,rock_unit_data$rock_unit_database$rock_no)];
#					poss_sites$province <- rock_unit_data$rock_unit_database$province[match(poss_sites$rock_no_sr,rock_unit_data$rock_unit_database$rock_no)]
#					good_sites <- unique(rbind(poss_sites[poss_sites$region %in% fuzzy_ranges$region[txn],],poss_sites[poss_sites$province %in% fuzzy_ranges$region[txn],]));
#					good_finds <- poss_finds[poss_finds$collection_no %in% good_sites$collection_no,];
#					good_finds$accepted_name <- missing_progeny[mp];
#					accepted_names <- c(accepted_names,missing_progeny[mp]);
#					find_data <- rbind(find_data,good_finds);
#					}
...
#					anagenetic_ancestors[anc] <- 1;
#				}
			} # end search for co-occurring AD pairs
...
phylo_axis <- phylo_axis_info$phylo_axis;
anagenetic_chains <- phylo_axis_info$anagenetic_chains;
...
an <- 0;
dummy <- data.frame(ancestor=as.character("a"),descendant=as.character(""),
...
while (an<length(anagenetic_chains))	{
	...
	this_chain <- anagenetic_chains[[an]];
	all_anagenetics <- c(all_anagenetics,this_chain);
	lchain <- length(this_chain);
	lineage <- this_chain[this_chain<=notu];
	nlineage <- length(lineage);
	clades <- this_chain[this_chain>notu];
	anagenetic_fuzzies$ancestor[an] <- otu_names[this_chain[1]];
	if (length(lineage)>1)
		anagenetic_fuzzies$descendant[an] <- paste(otu_names[lineage[2:nlineage]],collapse="->");
#	if (length(clades)>0)
#		anagenetic_fuzzies$descendant[an] <- paste(anagenetic_fuzzies$descendant[an],names(phylo_axis)[clades],collapse="->");

	anc_no <- this_chain[1];
	dsc_no <- this_chain[lchain];
	anagenetic_fuzzies$fa_lb[an] <- fuzzy_ranges$fa_lb[anc_no];
	anagenetic_fuzzies$fa_ub[an] <- min(fuzzy_ranges$fa_ub[lineage]);
	anagenetic_fuzzies$la_lb[an] <- max(fuzzy_ranges$la_lb[lineage]);
	anagenetic_fuzzies$la_ub[an] <- min(fuzzy_ranges$la_ub[lineage]);
	anagenetic_fuzzies$anc_no[an] <- anc_no;
	anagenetic_fuzzies$dsc_no[an] <- dsc_no;
	anagenetic_fuzzies$taxon[an] <- paste(anagenetic_fuzzies$ancestor[an],"->",anagenetic_fuzzies$descendant[an],sep="");
	anagenetic_fuzzies$phylo_axis[an] <- this_axis <- phylo_axis[this_chain[1]];
	}

#write.csv(cbind(sampled_ancestors,anagenetic_ancestors),"Ancestors.csv",row.names = TRUE);
#write.table(tree_output$newick_modified,"Newick.txt",quote=FALSE,row.names = FALSE);
...
names(anagenetic_overlaps)[anagenetic_overlaps>0] <- sort(unique(phylo_axis[1:notu]));
...
while (an<length(anagenetic_overlaps))	{
	...
	this_axis <- phylo_axis[match(anagenetic_fuzzies$ancestor[an],names(phylo_axis))];
	morphospc <- (1:notu)[phylo_axis[1:notu]==this_axis];
#	otu_names[morphospc]
...
#		if ()
		...
		ch <- 1;
		while (anagenetic_chains[[ch]][1]!=sp)	ch <- ch+1;
		for (dd in 1:length(anagenetic_chains[[ch]]))	{
			spc <- anagenetic_chains[[ch]][dd];
			if (spc<=notu)	{
				rect(-abs(fuzzy_ranges$fa_lb[spc]),phylo_axis[sp]-1/3,-abs(fuzzy_ranges$la_ub[spc]),phylo_axis[sp]+1/3,col=makeTransparent(otu_colors[spc],100),border=makeTransparent(otu_colors[spc],100),lwd=0);
				if (min_ranges[spc,1]!=0)
					rect(-abs(min_ranges$fa_mr[spc]),phylo_axis[spc]-1/3,-abs(min_ranges$la_mr[spc]),phylo_axis[spc]+1/3,col=otu_colors[sp],border=otu_colors[spc],lwd=0);
				}
			}
		}
	}
...
clade_posteriors[clade_posteriors<0.2] <- 0.2;
...
#			text(xx,phylo_axis[sp],anagenetic_fuzzies$taxon[fuzzy_dude],pos=4,cex=otu_font_size,family=franky);
			...
			af <- match(sp,anagenetic_fuzzies$anc_no);
			xx <- -anagenetic_fuzzies$la_ub[af];
#			xx <- max(branch_ranges$finish[sp],-abs(fuzzy_ranges$la_ub[sp]));
			text(xx,phylo_axis[sp],anagenetic_fuzzies$taxon[af],pos=4,cex=otu_font_size,family=franky);

...
mctree_file <- paste(tree_drawing_folder,"NoCheiro_mcc.tre",sep="");
rank_to_plot <- "order";	# choose which taxonomic rank you want to print; irrelevant if you don't choose variable_to_plot <- "taxonomy"
...
study_ma_ub <- time_scale$ma_ub[time_scale$interval==study_end];
use_PBBD_for_cooccr_anc_desc <- TRUE;
...
if (taxon_information_file=="")  {
  ...
  otu_information$taxon_type <- rep("ingroup",notu);
	...
	while (bg < length(informals))	{
		bg <- bg+1;
		tx <- informals[bg];
		otu_information$genus[tx] <- divido_genus_names_from_species_names(otu_information$taxon[tx]);
...
#	cinctan_finds <- read.csv(paste(this_directory,"Cincta_Finds.csv",sep=""),header=TRUE,stringsAsFactors = FALSE);
	...
	#	fuzzy_ranges[desc,]
      ...
      anagenetic_sets_no <- rbind(anagenetic_sets_no,dummy_set_no);
		...
		if (use_PBBD_for_cooccr_anc_desc)	{

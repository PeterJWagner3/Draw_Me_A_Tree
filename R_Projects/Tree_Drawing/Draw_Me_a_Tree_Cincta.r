#### Setup Program ####
this_directory <- "~/Documents/R_Projects/Diversification_for_RevBayes/";
...
#### Load Data ####
diversification_for_revbayes <- "~/Documents/R_Projects/Diversification_for_RevBayes/";
...
range_file <- paste(diversification_for_revbayes,"Cinctan_Fuzzy_Ranges.csv",sep="");
...
basal_date_raw <- max(((tree_output$hpd$lb+tree_output$hpd$ub)/2)[1:notu]);
...
cinctan_family_assignments <- read.csv("Cinctan_Families.csv",header=T,stringsAsFactors = F);
cinctan_families <- unique(cinctan_family_assignments$Family);
family_colors <- c("gray50","orange","dodgerblue","purple","orange1");
...
rownames(branch_ranges) <- names(branch_durations);
...
if (length(ancestral_spc)>0)	{
	...
	cinctan_finds <- read.csv(paste(diversification_for_revbayes,"Cincta_Finds.csv",sep=""),header=T,stringsAsFactors = F);
	...
	#	fuzzy_ranges[desc,]
		...
		anc_find_data <- subset(cinctan_finds,cinctan_finds$accepted_name==otu_names[anc])
		if(sum(abs(fuzzy_ranges$la_lb[anc])<=abs(fuzzy_ranges$fa_ub[progeny]))==0)	{
			find_data <- subset(cinctan_finds,cinctan_finds$accepted_name %in% otu_names[c(anc,progeny)]);
...
if (length(anagenetic_overlaps)>0)
...
#### plot time scale ####
st <- sum(max(abs(branch_ranges$start)) <= abs(finest_chronostrat$ma_lb));
en <- sum(min(abs(fuzzy_ranges$la_ub)) <= abs(finest_chronostrat$ma_ub))
...
strat_names[strat_names==c("Dp2,Ka3")] <- "";
...
time_scale_to_plot[1] <- -abs(gradstein_2012_emended$time_scale$ma_lb[match(oldest_interval,gradstein_2012_emended$time_scale$interval)]);
# Do the same with the oldest date
time_scale_to_plot[length(time_scale_to_plot)] <- -abs(gradstein_2012_emended$time_scale$ma_ub[match(youngest_interval,gradstein_2012_emended$time_scale$interval)]);
...
#mny <- 0;
strat_label_size <- 1.5;					# size of the labels for chronostratigraphic units
yearbreaks <- c(1,0.5,0.1);
yearbreaks <- sort(yearbreaks);
otu_colors <- family_colors[match(cinctan_family_assignments$Family,cinctan_families)];
otu_colors[is.na(otu_colors)] <- "gray80";
print_names <- T;
print_pix <- F;
#### Plot Tree ####
Phanerozoic_Timescale_Plot_Flexible(onset,end+2,time_scale_to_plot,mxy,mny=(mny-(mxy-mny)/20),use_strat_labels,strat_names,strat_colors,plot_title,ordinate,abscissa="Ma",yearbreaks,xsize=xsize,ysize=ysize,hues=hues,colored=colored,alt_back=alt_back,strat_label_size=strat_label_size);
for (sp in 1:notu)	{
#	sp <- sp+1;
...
for (an in 1:length(ancestral_spc))	observed_nodes_all[which(mat_tree==ancestral_spc[an],arr.ind = T)[1]] <- ancestral_spc[an];
...
top <- mxy+1;
for (ii in 2:4)	{
	rect(onset,top,onset+1,top-1,col=family_colors[ii]);
	text(onset+1,top-0.5,cinctan_families[ii],pos=4);
	top <- top-1.25;
	}
if (print_names)	{
	sp <- 0;
#	for (sp in 1:notu)	{
	while (sp < notu)	{
		...
		if (!sp %in% anagenetic_fuzzies$anc_no && !sp %in% anagenetic_fuzzies$dsc_no)	{
			...
			text(xx,phylo_axis[sp],otu_names[sp],pos=4,cex=0.75)
			} else	if (sp %in% anagenetic_fuzzies$dsc_no)	{
			fuzzy_dude <- match(sp,anagenetic_fuzzies$dsc_no);
#		lineage_name <- paste(anagenetic_fuzzies$ancestor[fuzzy_dude],"->",anagenetic_fuzzies$descendant[fuzzy_dude],sep="")
			xx <- max(branch_ranges$finish[sp],-abs(fuzzy_ranges$la_ub[sp]));
			text(xx,phylo_axis[sp],anagenetic_fuzzies$taxon[fuzzy_dude],pos=4,cex=0.75)
			}
		}
	}
if (print_pix)	{
	...
	for (pc in 1:length(pictured))	{
		pic_file <- paste(diversification_for_revbayes,pictured[pc],"_horizontal.png",sep="");
...
cinctan_family_nodes <- vector(length=length(cinctan_families));
names(cinctan_family_nodes) <- cinctan_families;
...
#		} else	{
		temp_col <- makeTransparent(family_colors[i],alpha=100);
		rect(hpd$lb[cfn],phylo_axis[cfn]-0.125,hpd$ub[cfn],phylo_axis[cfn]+0.125,col=temp_col,lwd=0);
#		}
	}
#for (pc in 1:length(pictured))	{
#	jpg_file <- paste(diversification_for_revbayes,pictured[pc],"_horizontal.jpg",sep="");
#	txn <- match(pictured[pc],otu_names_nex);
